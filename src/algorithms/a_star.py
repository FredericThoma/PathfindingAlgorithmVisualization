...
class AStar(AlgorithmInterface):
    ...
    def initialize(self, maze):
        ...
        maze.start_cell.parent = maze.start_cell
        maze.start_cell.calc_dists(maze.start_cell.parent, maze)
    ...
    def next_step(self, current_cell, maze):
        ...
        elif current_cell is maze.start_cell and not self.initialized:
    ...
    def find_min_in_open_nodes(self):
        cells_with_min_h_cost = []
        min_f_cost = float('inf')
        min_h_cost = float('inf')
        for cell in self.open_nodes:
            if cell.f_cost == min_f_cost:
                cells_with_min_h_cost.append(cell)
            if cell.f_cost < min_f_cost:
                cells_with_min_h_cost.clear()
                cells_with_min_h_cost.append(cell)
                min_f_cost = cell.f_cost
        if len(cells_with_min_h_cost) == 1:
            return cells_with_min_h_cost[0]
        else:
            current_cell = None
            for cell in cells_with_min_h_cost:
